...
namespace solutio
{
  ...
  std::vector<double> BrachyPlan::CalculateDose(BrachyDoseTG43 &bdc,
    Vec3<double> point, bool line_source)
  {
    std::vector<double> results;
    int counter = 0;
    double dose_sum = 0.0, min_radius = 30.0, max_radius = -0.1, ave_radius = 0.0,
      min_theta = 200.0, max_theta = -0.1, ave_theta = 0.0;
    Vec3<double> dist, direction;
    for(int a = 0; a < Applicators.size(); a++)
    {
      for(int c = 0; c < Applicators[a].Channels.size(); c++)
      {
        double prev = 0.0;
        int ind = Applicators[a].Channels[c].ControlPoints.size()-1;
        direction = Applicators[a].Channels[c].ControlPoints[ind].Position -
          Applicators[a].Channels[c].ControlPoints[0].Position;
        direction.Normalize();
        for(int p = 0; p < Applicators[a].Channels[c].ControlPoints.size(); p++)
        {
          dist = point - Applicators[a].Channels[c].ControlPoints[p].Position;
          double r = dist.Magnitude() / 10.0;
          ave_radius += r;
          if(r > max_radius) max_radius = r;
          if(r < min_radius) min_radius = r;

          double weight = Applicators[a].Channels[c].ControlPoints[p].Weight - prev;
          prev = Applicators[a].Channels[c].ControlPoints[p].Weight;
          double dwell_time = Applicators[a].Channels[c].TotalTime*
            (weight / Applicators[a].Channels[c].FinalCumulativeTimeWeight);

          int sid = Applicators[a].Channels[c].ReferencedSourceNumber;
          double aks = Sources[sid].Strength;
          ...
          if(line_source)
          {
            ...
            dose_sum += (dwell_time/3600.0)*bdc.CalcDoseRateLine(aks, r, theta);
          }
          else dose_sum += (dwell_time/3600.0)*bdc.CalcDoseRatePoint(aks, r);
          counter++;
        }
      }
    }
    ave_radius /= double(counter);
    ave_theta /= double(counter);
    results.push_back(dose_sum);
    results.push_back(min_radius);
    results.push_back(max_radius);
    results.push_back(ave_radius);
    results.push_back(min_theta);
    results.push_back(max_theta);
    results.push_back(ave_theta);
    return results;
  }
}
