...
template <class T>
T LinearInterpolation(std::vector<T> x_data, std::vector<T> y_data, std::vector< std::vector<T> > table, T x_value, T y_value){
  int index = 0;
  while((index < x_data.size()) && (x_value >= x_data[index])) index++;
  if(index <= 0) index = 1;
  if(index >= x_data.size()) index = x_data.size()-1;
...
template <class T>
T LinearInterpolationFast(std::vector<T> x_data, std::vector<T> y_data, T x_value, T delta_x){
...
template <class T>
T LinearInterpolationFast(std::vector<T> x_data, std::vector<T> y_data, std::vector< std::vector<T> > table, T x_value, T y_value, T delta_x, T delta_y){
  int index = ceil((x_value-x_data[0])/delta_x);
  if(index <= 0) index = 1;
  if(index >= x_data.size()) index = x_data.size()-1;
  float y_1 = LinearInterpolationQuick(y_data, table[(index-1)], y_value, delta_y);
  float y_2 = LinearInterpolationQuick(y_data, table[index], y_value, delta_y);
  T f = (x_value - x_data[(index-1)]) / (x_data[index] - x_data[(index-1)]);
  T t_value = f*y_2 + (1-f)*y_1;
  return t_value;
}
